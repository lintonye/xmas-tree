...
const Background = props => (
  <BackgroundImg src={Images.background} {...props} />
);

const Foreground = props => (
  <ForegroundImg src={Images.foreground} {...props} />
);
...
const Superman = ({ status }) => {
  ...
  switch (status) {
    case "bored":
      img = Images.supermanBored;
      break;
    case "magic":
      img = Images.supermanMagic;
      break;
    case "celebrate":
      img = Images.supermanCelebrate;
      break;
    default:
      img = Images.supermanBored;
  }
  ...
  animate = () => {
    ...
    animation
      ...
      .to(
        this.boxLid,
        1,
        { x: 20, y: -100, scale: 4, ease: Power3.easeInOut },
        "-=0.5"
      )
      .to(
        this.boxBody,
        1,
        { x: 20, y: 50, scale: 4, ease: Power3.easeInOut },
        "-=1"
      )
...
const Gift = ({ onClick, isOpen }) => (
  <GiftImg
    onClick={onClick}
    src={isOpen ? Images.giftUnwrapping : Images.gift}
  />
);
...
class Scene extends Component {
  ...
  componentDidMount() {
    this._checkWateringInterval = setInterval(() => {
      ...
      const increaseTreeAge =
        this._isWateringTree() &&
        now - this._lastTimeStamp > 2000 &&
        this.state.treeAge < TREE_MAX_AGE;
  ...
  render() {
    ...
    const supermanStatus = showXmasTree
      ? "celebrate"
      : this._isWateringTree()
      ? "magic"
      : "bored";
    ...
    return (
      <SceneDiv
        ...
        onMouseMove={_.throttle(this.onMouseMove, 200)}
        onClick={this._updateXY}
        hideCursor={!showXmasTree}
      >
        ...
        <MidSceneDiv offsetX={parallax.mid}>
          ...
          {showXmasTree && (
            <Gift
              onClick={this._handleGiftClick}
              isOpen={this.state.giftOpen}
            />
          )}
          {showXmasTree && !this.state.giftOpen && (
            <OpenGiftImg src={Images.open} />
          )}
        </MidSceneDiv>
        {this._shouldWaterComeOut() && !showXmasTree && (
          <Water x={this.state.mouseX} y={this.state.mouseY} />
        )}
        {!showXmasTree && (
          <Waterpot
            x={this.state.mouseX}
            y={this.state.mouseY}
            rotate={this._shouldWaterComeOut()}
          />
        )}
        <Foreground offsetX={parallax.near} />
        <Sound
          url={showXmasTree ? Sounds.xmas : Sounds.background}
          loop={true}
        />
